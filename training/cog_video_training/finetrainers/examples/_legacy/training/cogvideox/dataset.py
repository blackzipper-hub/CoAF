...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if self.load_tensors:
            ...
            image, video, _ = self._preprocess_video(self.video_paths[index], self.image_paths[index])
    ...
    def _load_dataset_from_local_path(self) -> Tuple[List[str], List[str], List[Optional[Path]]]:
        ...
        if self.image_column:
            image_path = self.data_root.joinpath(self.image_column)
            if not image_path.exists() or not image_path.is_file():
                raise ValueError(
                    "Expected `--image_column` to be path to a file in `--data_root` containing line-separated paths to condition images."
                )
            with open(image_path, "r", encoding="utf-8") as file:
                image_paths = [self.data_root.joinpath(line.strip()) for line in file.readlines() if len(line.strip()) > 0]
            if len(image_paths) != len(video_paths):
                raise ValueError(
                    f"Expected length of images and videos to be the same but found {len(image_paths)=} and {len(video_paths)=}."
                )
        ...
        if self.image_to_video and self.image_column and any(not path.is_file() for path in image_paths):
            raise ValueError(
                f"Expected `{self.image_column=}` to contain valid condition image paths but found atleast one invalid path."
            )

        return prompts, video_paths, image_paths

    def _load_dataset_from_csv(self) -> Tuple[List[str], List[str], List[Optional[Path]]]:
    ...
    def _preprocess_condition_image(self, path: Path, height: int, width: int) -> torch.Tensor:
        image = Image.open(path.as_posix()).convert("RGB")
        image = TT.functional.pil_to_tensor(image).float()
        image = resize(image, [height, width], interpolation=InterpolationMode.BICUBIC)
        image = self.video_transforms(image)
        return image.unsqueeze(0)

    def _preprocess_video(self, path: Path, image_path: Optional[Path] = None) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        r"""
        ...
        if self.load_tensors:
            ...
            image = (
                self._preprocess_condition_image(image_path, frames.shape[2], frames.shape[3])
                if self.image_to_video and image_path is not None
                else frames[:1].clone()
...
class VideoDatasetWithResizing(VideoDataset):
    ...
    def _preprocess_video(self, path: Path, image_path: Optional[Path] = None) -> torch.Tensor:
        if self.load_tensors:
            return self._load_preprocessed_latents_and_embeds(path)
        else:
            video_reader = decord.VideoReader(uri=path.as_posix())
